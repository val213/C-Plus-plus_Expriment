	...
	AnimalEnclosure animalenclosure_elephant, AnimalEnclosure animalenclosure_giraffe, AnimalEnclosure animalenclosure_monkey,int t)
{
	...
	*/
	...
	for (int n = 0; n < num_of_adult_a_day; n++)
	{
		...
		else  if ((t % 2) == (animalenclosure_elephant.opstate_judge_get() % 2)) animalenclosure_elephant.open();
			//如果围栏还开着、动物吃得下，喂食，每个动物围栏变脏，每一只动物吃的食物增加
	...
	if (animalenclosure_elephant.getDirt() >= 1000)
	{
	...
	if (animalenclosure_giraffe.getDirt() >= 1000)
	{
	...
	if (animalenclosure_monkey.getDirt() >= 1000)
	{
...
Money FoodSeller::getMoney() const
{
	return Money();
}
